...
Module::Module(int id, int layer, bool horizontal, float x, float y, float z){
	...
	ToT1 = new TH1F(TString("ToT_module_") + Convert::toNdigit(2*id, 3), "", 200, 0, 50);
	ToT1->GetXaxis()->SetTitle("ToT (arb Units)");
	ToT2 = new TH1F(TString("ToT_module_") + Convert::toNdigit(2*id+1, 3), "", 200, 0, 50);
	ToT2->GetXaxis()->SetTitle("ToT (arb Units)");
